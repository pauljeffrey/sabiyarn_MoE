...
**Dry run / checkpoint sanity check** (`training.test_run: true` in `train_config.yaml`): loads the model and `model.reference_repo`, runs the weight-deviation check, then generates from 5 fixed prompts (one per pretraining language, ~20-30 words each, 150 new tokens) with **both** models under **both** `do_sample` and beam search, prints them side by side, and stops before the first training step — nothing is trained, checkpointed, or pushed. Set it back to `false` to train. The prompts live in `Trainer._STARTUP_PROMPTS` (`training/new_train.py`); edit them freely. The comparison itself runs at the start of *every* launch — only the stop-afterwards part is gated on `test_run`.
